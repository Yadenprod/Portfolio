<template>
  <div class="min-h-screen bg-azot-gradient flex flex-col items-center justify-start py-12 px-4 sm:px-8 relative overflow-hidden">
    ...
    <transition name="fade" mode="out-in">
      <div v-if="!loading" key="equipment" class="relative z-10 w-full max-w-6xl animate-wow-fade-in">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-10 gap-6">
          <h1 class="text-4xl md:text-5xl font-extrabold text-azot-700 dark:text-azot-200 mb-2 tracking-tight animate-wow-fade-in">Оборудование</h1>
        ...
        <div class="bg-white/90 dark:bg-industrial-900/90 rounded-wow shadow-glow-azot p-8 animate-wow-fade-in">
          <div class="mb-4 flex flex-col md:flex-row gap-2 md:gap-4 items-center">
            <input v-model="search" type="text" placeholder="Поиск по названию..." class="input" />
            ...
            <button @click="sortAsc = !sortAsc" class="btn-primary">{{ sortAsc ? '↑' : '↓' }}</button>
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-industrial-200 dark:divide-industrial-700 rounded-xl overflow-hidden">
              <thead class="bg-industrial-50 dark:bg-industrial-900">
                <tr>
                  <th class="px-4 py-2 text-left text-xs font-medium text-industrial-500 dark:text-industrial-400 uppercase">Название</th>
                  <th class="px-4 py-2 text-left text-xs font-medium text-industrial-500 dark:text-industrial-400 uppercase">Локация</th>
                  <th class="px-4 py-2 text-left text-xs font-medium text-industrial-500 dark:text-industrial-400 uppercase">Статус</th>
                  <th class="px-4 py-2"></th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="item in paginatedEquipment" :key="item?.id ?? Math.random()" class="hover:bg-azot-50 dark:hover:bg-industrial-800 transition animate-wow-fade-in">
                  <td class="px-4 py-2 text-industrial-900 dark:text-industrial-100 font-medium">{{ item?.name ?? '-' }}</td>
                  <td class="px-4 py-2 text-industrial-500 dark:text-industrial-300">{{ item?.location ?? '-' }}</td>
                  <td class="px-4 py-2">
          ...
          <div v-if="totalPages > 1" class="flex justify-center mt-4 gap-2">
            <button v-for="page in totalPages" :key="page" @click="currentPage = page" :class="['px-3 py-1 rounded', currentPage === page ? 'bg-azot-600 text-white' : 'bg-industrial-200 dark:bg-industrial-700 text-industrial-700 dark:text-industrial-200']">{{ page }}</button>
          </div>
          <div v-if="!filteredEquipment.length" class="text-center text-industrial-400 py-8">Нет оборудования</div>
          <div v-if="error" class="text-center text-red-500 py-4">{{ error }}</div>
        </div>
        <EntityModal :open="modalOpen" :title="modalMode === 'create' ? 'Добавить оборудование' : 'Редактировать оборудование'" :onClose="closeModal" :onSave="save">
          <form @submit.prevent="save">
            <div class="mb-4">
              <label class="block text-industrial-700 dark:text-industrial-200 mb-1">Название</label>
              <input v-model="form.name" type="text" required class="input" />
            </div>
            <div class="mb-4">
              <label class="block text-industrial-700 dark:text-industrial-200 mb-1">Локация</label>
      ...
      <div v-else key="loader" class="flex flex-col items-center justify-center min-h-[60vh] w-full animate-wow-fade-in">
        ...
        <span class="mt-4 text-azot-600 text-lg font-semibold animate-wow-bounce">Загрузка оборудования...</span>
      </div>
    </transition>
  </div>
</template>
...
const form = ref({ id: null, name: '', location: '', status: 'working' });
...
const filteredEquipment = computed(() => {
  ...
  if (search.value) {
    result = result.filter(e => e.name.toLowerCase().includes(search.value.toLowerCase()));
  }
...
function openModal(mode, item = null) {
  ...
  if (mode === 'edit' && item) {
    ...
    form.value = { id: null, name: '', location: '', status: 'working' };
  }
...
async function save() {
  try {
    if (modalMode.value === 'create') {
      await axios.post('/api/equipment', form.value);
    } else {
      await axios.put(`/api/equipment/${form.value.id}`, form.value);
    }
...
function getStatusClass(status) {
  return {
    'inline-block px-2 py-1 rounded text-xs font-semibold bg-green-100 text-green-700': status === 'working',
    'inline-block px-2 py-1 rounded text-xs font-semibold bg-yellow-100 text-yellow-700': status === 'attention',
    'inline-block px-2 py-1 rounded text-xs font-semibold bg-red-100 text-red-700': status === 'critical',
  };
}
function exportCSV() {
  const rows = [
    ['ID', 'Название', 'Локация', 'Статус'],
    ...filteredEquipment.value.map(e => [e.id, e.name, e.location, e.status])
  ];
